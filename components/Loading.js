...
export default function Loading({ size = 'medium', color = 'blue' }) {
	...
	return (
		<svg height={size} width={size} className={styles.svg}>
			<circle className={`${styles[sizeStr]} ${styles[color]} ${styles.circle}`} cx={size / 2} cy={size / 2} r={size / 3} strokeLinecap="round"></circle>
		</svg>
	);
}